...
const AddDataAppointment = ({navigation, route}) => {
...
const handleAdd = async () => {
   ...
   const getFindself =  await AsyncStorage.getItem('@findSelf')
    ...
    .then((resDB) => {
      Fire.database()
      .ref('leads/' + data.idLeads + '/')
      .update(dataUpdateLeads)
        navigation.replace('MyTabs')
        alert('Berhasil Tambah Appointment')
    })
}
useEffect(() => {
    gettoken()
  ...
  return (
    ...
    <View style={{flex: 1, backgroundColor: 'white'}}>
     ...
     <ScrollView>
     <View >
        <Text style={{color: 'black', textAlign: 'center', fontWeight: 'bold', fontSize: 20, marginTop: 20}}>Add Data Appointment</Text>
      ...
      onPress={() => setOpen(true)}>
        ...
        :
        <Text style={{fontSize: 14, color: 'grey', textAlign: 'center'}}>Pilih Tanggal Follow UP 1</Text>
...
export default AddDataAppointment
